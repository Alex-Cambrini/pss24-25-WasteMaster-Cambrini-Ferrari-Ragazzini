...
class TripDAOTest extends AbstractDatabaseTest {
    ...
    public void setUp() {
        super.setUp();
        tripStart = LocalDateTime.now().plusDays(3).withHour(8).withMinute(0).withSecond(0).withNano(0);
    ...
    void testFindAvailableVehicles() {
        Vehicle v1 = new Vehicle("AA111AA", "Iveco", "Daily",
                LocalDate.now().getYear(), RequiredLicence.C, VehicleStatus.IN_SERVICE, 2);
        ...
        Vehicle v3 = new Vehicle("CC333CC", "Fiat", "Ducato",
                LocalDate.now().getYear(), RequiredLicence.C1, VehicleStatus.OUT_OF_SERVICE, 1);
        getVehicleDAO().insert(v3);

        Vehicle v4 = new Vehicle("DD444DD", "MAN", "TGL",
                LocalDate.now().getYear(), RequiredLicence.C, VehicleStatus.IN_SERVICE, 2);
        v4.setNextMaintenanceDate(tripEnd.toLocalDate().minusDays(1)); // nextMaint < tripEnd → out
    ...
    void testFindAvailableOperators() {
        ...
        Employee e1 = new Employee("Mario", "Verdi", addr, "mario.verdi@example.com", "3331111111", Role.OPERATOR, Licence.C);
        Employee e2 = new Employee("Luigi", "Bianchi", addr, "luigi.bianchi@example.com", "3332222222", Role.OPERATOR, Licence.C);
        ...
        List<Employee> available = getTripDAO().findAvailableOperators(tripStart, tripEnd);

        assertTrue(available.stream().anyMatch(op -> op.getEmployeeId().equals(e1.getEmployeeId())));
        assertFalse(available.stream().anyMatch(op -> op.getEmployeeId().equals(e2.getEmployeeId())));
    }
    ...
    void testFindAvailablePostalCodes() {
        ...
        Customer c1 = new Customer("Mario", "Rossi", loc1, "mario.rossi@example.com", "3330000001");
        Customer c2 = new Customer("Giulia", "Neri", loc2, "giulia.neri@example.com", "3330000002");
