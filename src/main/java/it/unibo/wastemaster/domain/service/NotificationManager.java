...
public class NotificationManager {
     ...
     */
    public NotificationManager(final TripRepository tripRepository,
     ...
     */
    private static String formatEur(final double amount) {
    ...
    /**
     * Retrieves the 5 most recent notifications aggregated from customers, invoices,
     * and trips.
     * Timestamps used:
     * - Customers: createdDate
     * - Invoices: paymentDate if present, otherwise issueDate
     * - Trips: departureTime
     * Events are merged, sorted descending by timestamp, and converted into
     * Notification objects.
     * The first 5 items are returned.
     *
     * @return a list of up to 5 latest notifications based on the timestamps listed above
     */
    public List<Notification> getLast5Events() {
        ...
        invoiceRepository.findLast5InvoicesEvent()
                .forEach(i -> {
                    ...
                    String customerName = (i.getCustomer() != null)
                            ...
                            : "Unknown customer";
                    String statusText =
                            (i.getPaymentStatus() == Invoice.PaymentStatus.PAID)
                                    ? "Paid"
                                    : "Not Paid";
        ...
        return events.stream().limit(MAX_NOTIFICATIONS).toList();
    }
}
